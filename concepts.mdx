...
Every span carries an OpenInference span kind (`openinference.span.kind`) so bryel knows what it is. The adapters set this for you; a custom [`SpanMapper`](/api-reference/sdk-core#spanmapper) sets it from [`OpenInferenceSpanKind`](/api-reference/semconv).
...
<Note>
  Span kinds and the `bryel.*` attributes are defined in [`@bryel/semconv`](/api-reference/semconv).
</Note>
